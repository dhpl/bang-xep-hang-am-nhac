...
public class FragmentNhat extends Fragment implements AsyncRespone{

    private static final String NHAC_NHAT = "http://chiasenhac.vn/mp3/japan/";

    public HienThiDanhSachBaiHat2 mHienThiDanhSachBaiHat;

    private RecyclerView mRecyclerViewNhacNhat;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_nhat, container, false);
        mHienThiDanhSachBaiHat = new HienThiDanhSachBaiHat2();
        mHienThiDanhSachBaiHat.delegate = this;
        mHienThiDanhSachBaiHat.execute(NHAC_NHAT);
        mBaiHats = new ArrayList<>();
        mRecyclerViewNhacNhat = (RecyclerView) view.findViewById(R.id.recyclerViewNhat);

        mRecyclerViewNhacNhat.setLayoutManager(new LinearLayoutManager(getContext(), LinearLayoutManager.VERTICAL, false));
        mRecyclerViewNhacNhat.setHasFixedSize(true);
        mRecyclerViewNhacNhat.setNestedScrollingEnabled(false);
        mRecyclerViewNhacNhat.setItemAnimator(new DefaultItemAnimator());
        return view;
    }
    ...
    public void processFinish(List<BaiHat> output) {
        ...
        mRecyclerViewNhacNhat.setAdapter(mBaiHatAdapter);
