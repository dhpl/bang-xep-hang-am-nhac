...
public class FragmentUSUK extends Fragment implements AsyncRespone{

    private static final String NHAC_US_UK = "http://chiasenhac.vn/mp3/us-uk/";

    public HienThiDanhSachBaiHat mHienThiDanhSachBaiHat;

    private RecyclerView mRecyclerViewNhacUSUK;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_us_uk, container, false);

        mHienThiDanhSachBaiHat = new HienThiDanhSachBaiHat();
        mHienThiDanhSachBaiHat.delegate = this;
        mHienThiDanhSachBaiHat.execute(NHAC_US_UK);
        mBaiHats = new ArrayList<>();
        mRecyclerViewNhacUSUK = (RecyclerView) view.findViewById(R.id.recyclerViewUSUK);

        mRecyclerViewNhacUSUK.setLayoutManager(new LinearLayoutManager(getContext(), LinearLayoutManager.VERTICAL, false));
        mRecyclerViewNhacUSUK.setHasFixedSize(true);
        mRecyclerViewNhacUSUK.setNestedScrollingEnabled(false);
        mRecyclerViewNhacUSUK.setItemAnimator(new DefaultItemAnimator());
    ...
    public void processFinish(List<BaiHat> output) {
        ...
        mRecyclerViewNhacUSUK.setAdapter(mBaiHatAdapter);
