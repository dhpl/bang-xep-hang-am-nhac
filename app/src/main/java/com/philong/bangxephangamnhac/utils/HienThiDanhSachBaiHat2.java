...
public class HienThiDanhSachBaiHat2 extends AsyncTask<String, Void, List<BaiHat>> {
    ...
    protected List<BaiHat> doInBackground(String... params) {
        try {
            ...
            for(Element element : elements){
                String linkBaiHat = element.select("div[class=text2]").select("a").attr("href");
                String tenBaiHat = element.select("div[class=text2]").select("a").text();
                String tenCaSi = element.select("p").text();
                baiHatList.add(new BaiHat(tenBaiHat, tenCaSi, linkBaiHat, ""));
            }
            return baiHatList;
        } catch (IOException e) {
            e.printStackTrace();
